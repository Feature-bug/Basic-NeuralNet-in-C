...
* Backprop, ok this I have no fuckin clue on how to do I know it uses the chain rule, how the fuck am i supposed to implement that in code! I guess I could apply a
  ...
  Update: I just the formula was wrong it was f(x) = (f(x + h) - f(x)) / h, where h tends to infinity.
*/
...
double mean_square_error(neuron *out_pred, double *out_real, int NumberofInput){
    double error = 0.0;
    for(int i = 0; i < NumberofInput; i++){
        error += pow(out_pred->weight[i] - out_real[i], 2);
    }
    return error / NumberofInput;
}

double regression(neuron *neuron, float *input, int NumberofInput){
    double out = 0.0;
    for(int i = 0; i < NumberofInput; i++){
        out += neuron->weight[i] * input[i];
    }
    return out + neuron->bias;
}

double gradient_descent(neuron *neuron, double error, int index){
    long alpha = 0.01;
    neuron->weight[index] -= alpha * error;
    //code
}
